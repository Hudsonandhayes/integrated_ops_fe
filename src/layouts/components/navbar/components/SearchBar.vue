	...
	<div class="flex">
		...
		>
			...
			>
				...
				<!--Clients Suggestion -->
				<template v-slot:clients="{ suggestion }">
					<div class="flex items-end leading-none py-1">
...
export default {
	...
	mounted() {
		axios
			.get("/clients/all")
			.then((res) => {
				this.navbarSearchAndPinList.clients.data = res.data.data;
			})
			.catch((err) => {
				console.log(err);
			});
	...
	methods: {
		selected(item) {
			...
			if (item.clients)
				this.$router
					.push("/pages/client/" + item.clients.id)
					.catch(() => {});
	...
	created() {
		// this.navbarSearchAndPinList.clients.data = ClientService.get_clients();
	},
};
</script>
