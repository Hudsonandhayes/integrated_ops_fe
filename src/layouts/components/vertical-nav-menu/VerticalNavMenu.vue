			...
			<div @mouseenter="mouseEnter" @mouseleave="mouseLeave">
				...
				<div class="header-sidebar flex items-end justify-between" slot="header">
					...
					<router-link tag="div" class="vx-logo cursor-pointer flex items-center" to="/">
						<img v-if="isMouseEnter"
							id="hnh-logo"
							alt="Hudson & Hayes logo"
							width="150px"
							src="@/assets/images/logo/logo.png"
						/>
						<!-- <logo class="w-32 mr-4 fill-current text-primary" /> -->
					</router-link>
					<!-- /Logo -->
...
export default {
			...
			for (const [index, item] of this.navMenuItems.entries()) {			
				...
				if (item.header && item.items.length && (index || 1)) {					
					...
					const i = clone.findIndex(
						(ix) => ix.header === item.header
					);
					for (const [subIndex, subItem] of item.items.entries()) {
						// console.log("item "+subItem.name+" roles "+subItem.roles+" role id "+this.role_id);
						if(subItem.submenu && subItem.submenu.length > 0){
							subItem.submenu.forEach((s_item,s_index) => {
								if(s_item.roles.includes(this.role_id)){
									clone.splice(i + 1 + s_index, 0, s_item);
								}
								 
							});
						}else{
							if(subItem.roles.includes(this.role_id)){
	...
	mounted() {
		// console.log("profile "+this.$store.state.profile.user);
		this.role_id = this.$store.state.profile.user.role_id || -1;
		this.setVerticalNavMenuWidth();
