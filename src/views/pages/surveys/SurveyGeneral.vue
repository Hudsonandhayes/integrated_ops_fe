...
export default {
		...
		return {
			...
			timelineData: [
				...
				},
				{
					...
					desc:
						"Setup Process mapping, Department Mapping and Employee associations",
					time: "15/11/2020",
				},
				{
					color: "danger",
				...
				},
				{
					...
					desc:
						"Deadline for participating employees to complete the survey",
					time: "10/12/2020",
				},
			],
	...
	methods: {
		...
		edit_survey() {
			this.$router.push({
				path: `/pages/client/${this.survey.client_id}/assessment/${this.survey_id}`,
			});
		},
		create_department() {
		...
		get_assessment_survey() {
			...
			this.survey = [];
			this.$vs.loading();
			SurveyService.get_assessment_survey(this.survey_id).then((response) => {
		...
		send_survey() {
			...
			SurveyService.send_survey(_that.survey_id).then((response) => {
