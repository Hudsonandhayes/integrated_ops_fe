	...
	<div>
		...
		<div class="vx-row">	 
			<div class="vx-col w-full">
				...
				<vx-card title="Process Timing Assessments">
					<template slot="actions">
						<vs-button
							@click.native="on_assessment_view(client_id, survey_id)"
							color="primary"
							class="mr-3"
							type="filled"
							key="assessment_create_btn"
							:disabled="!assessment_ready"							
							>Create Process Timing Survey</vs-button
					...
					<div slot="no-body" class="mx-2">
						...
						>
							...
							<template slot="thead">
								...
								<vs-th>Participants</vs-th>
								<vs-th>Survey Stage</vs-th>
								<vs-th>Status</vs-th>
								<vs-th>Action</vs-th>
							</template>
							...
							<template slot-scope="{ data }">
								...
								>
									...
									<vs-td :data="tr.start_date">{{tr.start_date | localdate }}</vs-td>
									<vs-td :data="tr.end_date">{{tr.end_date | localdate}}</vs-td>
									<vs-td :data="tr.no_of_employees">{{
										tr.no_of_employees
									}}</vs-td>
									<vs-td :data="tr.stage">{{
										tr.stage
...
export default {
	...
	methods: {
		...
		on_survey_view(client_id, id) {
			this.$router.push({
				path: `/pages/client/${client_id}/survey/${id}`,
			});
		},
		on_assessment_view(client_id, survey_id) {
			if(this.assessment_ready){
				...
				this.$vs.notify({
					...
					text: "No process tree or Resources created. Please create process tree and employees before starting a survey.",
					color: "warning",
					timing: 4000,				
				});	
			}
		...
		send_survey(id) {
			...
			this.$vs.loading();
			const _that = this;
			SurveyService.send_survey(id).then((response) => {
		...
		check_assessment_ready() {
			...
			ClientService.check_assessment_ready(this.client_id)
				.then((response) => {
					...
					if (output && output !== undefined) {
						if (output.success) {
							...
							_that.$vs.notify({
								title: "Get assessment readiness",
		...
		get_client_surveys() {
			...
			// this.employees = [];
			this.$vs.loading();
			SurveyService.get_client_surveys(this.client_id).then(
				(response) => {
					const data = response.data;
					if (data && data !== undefined) {
						if (data.success) {
